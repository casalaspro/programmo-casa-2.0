...
<div class="container my-3">
  ...
  <div class="selected-apartment_wrap">
    <div class="row justify-content-center">
      ...
      <div class="col-lg-8">
        <div class="form_wrap card p-3">
          <h4>Scegli la giusta sponsorizzazione per il tuo appartamento.</h4>
          <form class="card-body d-flex flex-column" action="{{ route('admin.payment.info', $apartment)}}" method="POST">
            @csrf
            @foreach ($sponsorships as $sponsorship)
              <div class="form-check">
                ...
                @if ($sponsorship->id === 1)
                  <input class="form-check-input" type="radio" name="sponsorship" value="{{$sponsorship->id}}" id="sponsor- {{$sponsorship->id}}" checked>
                @else
                  <input class="form-check-input" type="radio" name="sponsorship" value="{{$sponsorship->id}}" id="sponsor- {{$sponsorship->id}}">
                @endif
            ...
            <button class="btn btn-success ms-auto"> Vai al pagamento</button>
          </form>
        </div>
      </div>
